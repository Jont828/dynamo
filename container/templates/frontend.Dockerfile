...
# Install runtime dependencies (common + frontend + planner).
# Frontend needs tritonclient and its grpcio/protobuf constraints for gRPC serving.
# Planner/profiler deps are included because the DGDR operator uses the frontend
# image to run profiling jobs (which need kubernetes_asyncio, aiconfigurator, etc.).
