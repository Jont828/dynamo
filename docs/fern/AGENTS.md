...
The Python and Rust pages under `pages/reference/api/` and the Kubernetes
`pages/reference/kubernetes-api/full-api-reference.mdx` page are generated from source outside
`docs/`. Each carries a `GENERATED by … do not edit` marker. To change one, edit the source and rerun
its generator, never the page.
