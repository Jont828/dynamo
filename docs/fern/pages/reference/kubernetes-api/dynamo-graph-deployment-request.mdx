...
For the task-oriented walkthrough — authoring a request step by step — see [Auto Deploy with DGDR](../../kubernetes/auto-deployment/auto-deploy-with-dgdr.md). For ready-to-copy manifests, see [DGDR Templates](../../recipes/examples/dgdr.mdx).
...
|---|---|---|
| vLLM | `Frontend`, `VllmDecodeWorker` | `Frontend`, `VllmDecodeWorker`, `VllmPrefillWorker` |
...
`metadata.name` selects the final DGD name, and other identity metadata is ignored. `status`, null
...
For worked examples, see [DGDR Templates](../../recipes/examples/dgdr.mdx). For router configuration and EPP
...
<CardGroup cols={2}>
  ...
  <Card title="DGDR Templates" icon="file-code" href="../../recipes/examples/dgdr.mdx">
