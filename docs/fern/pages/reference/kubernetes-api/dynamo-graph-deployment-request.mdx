...
<Indent>
  ...
  <ParamField path="trustRemoteCode" type="boolean" default="false">
    Explicitly permits generated vLLM and SGLang workers to execute custom code from the configured model repository. The profiler adds `--trust-remote-code` to every worker component after the aggregate or disaggregated topology is known. Enable this setting only for model repositories you trust.
  </ParamField>
...
spec:
  ...
  overrides:
    dgd:
      ...
      spec:
        components:
        - name: SGLangDecodeWorker  # replace with the generated component name
          podTemplate:
            spec:
              containers:
              - name: main
...
Inspect `.status.profilingResults.selectedConfig` with `autoApply: false` to find component names.
