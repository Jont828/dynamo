...
<Steps toc={true}>
<Step title="Clone the repository and set working directory" id="clone-the-repository-and-set-working-directory">
...
<Step title="Setup environment variables" id="setup-environment-variables">
...
<Step title="Install CLIs" id="install-clis">
...
<Step title="Create an EKS Auto Mode cluster" id="create-an-eks-auto-mode-cluster">
...
<Step title="Create a default StorageClass" id="create-a-default-storageclass">
...
<Step title="Create an Amazon EFS shared file system" id="create-an-amazon-efs-shared-file-system">
Follow the [EFS setup guide](../../model-storage/efs.mdx) to create an EFS file system and make it available as shared storage for Dynamo workloads.
...
<Step title="Deploy a Dynamo DynamoGraphDeployment (DGD)" id="deploy-a-dynamo-dynamographdeployment-dgd">
...
It targets `g7e.12xlarge` instances, which support GPUDirect RDMA, and uses the Dynamo EFA-enabled vLLM container `nvcr.io/nvidia/ai-dynamo/vllm-runtime:1.3.0-efa` that ships with the [EFA Installer](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/efa-changelog.html) pre-installed.
...
vllm-disagg-decode-510a1741-7666987b-tp58w              1/1     Running   0          2m13s
vllm-disagg-prefill-510a1741-54f76d7954-tjgn8           1/1     Running   0          2m13s
```
...
vllm-agg-decode-d0a70291-759df94478-8lc74               1/1     Running   0          4m46s
```
...
Cleanup EFS related resources, follow the [EFS setup guide](../../model-storage/efs.mdx#cleanup) cleanup section
