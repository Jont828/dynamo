...
| `sla` | Planner engine queries backed directly by the AIC core wheel, targeting specific TTFT/ITL values | Yes | Recommended |
...
| `prefill_min_endpoint` | `null` | Prefill replica floor for disaggregated or prefill-only mode; replaces the prefill value from `min_endpoint` when set. |
| `decode_min_endpoint` | `null` | Decode replica floor for disaggregated or decode-only mode; replaces the decode value from `min_endpoint` when set. |
...
The endpoint floors and `load_scaling_down_sensitivity` matter most because of how scale-down works: when the Planner removes a worker, it terminates it **without draining in-flight requests**, so requests mid-prefill on that worker fail. In disaggregated deployments this can also fail decode workers waiting on a KV-cache transfer from the terminated prefill worker. To reduce the blast radius, set each component floor above its steady-state requirement and lower `load_scaling_down_sensitivity` so scale-down fires less often. See [Current Limitations](../../developer-guide/knowledge-base/modular-components/planner/overview.md#current-limitations) for the full caveat.
