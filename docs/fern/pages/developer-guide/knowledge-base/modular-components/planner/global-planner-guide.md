...
> **New to Planner?** We recommend starting with a single-DGD deployment using either throughput-based or load-based scaling before adopting GlobalPlanner. See the [Planner overview](overview.md) and [Planner Guide](planner-guide.md) to get started.
...
- **Planner**: The `dynamo.planner` component that computes desired replica counts to maintain latency SLAs. See the [Planner overview](overview.md).
...
- Dynamo Kubernetes Platform installed. See [Kubernetes Quickstart](../../../../kubernetes/getting-started/quickstart.mdx).
...
See [DGDR Templates](../../../../recipes/examples/dgdr.mdx) and [Profiler Guide](../profiler/profiler-guide.md) for DGDR details.
...
`throughput_metrics_source: "router"` is required for pool-local Planner in GlobalPlanner deployments. The pool Planner should forecast demand from its own `LocalRouter` `dynamo_component_router_*` Prometheus metrics, not from the shared public `Frontend`. See the [Planner overview](overview.md#prometheus-metrics) for the exact frontend and router metric sources.
...
- [Planner README](overview.md) — Planner overview and quick start
- [Planner Guide](planner-guide.md) — Planner configuration reference
- [DGDR Templates](../../../../recipes/examples/dgdr.mdx) — DGDR examples for generating per-pool configs
