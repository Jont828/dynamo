...
# vllm-omni-image-dgd.yaml: deploy the Qwen-Image text-to-image DGD, wait for Ready,
# generate an image, and show it. Narration types itself; ENTER types each command,
# ENTER again runs it.
#
#   ./vllm-omni-image-demo.sh                        # present against the pre-deployed DGD
#   SIMULATE_DEPLOY=false ./vllm-omni-image-demo.sh  # really apply and wait for Ready
#   ./vllm-omni-image-demo.sh -d -n                  # rehearse: no typing, no pauses (-w5 auto-advances)
#   ./vllm-omni-image-demo.sh cleanup                # delete the DGD and release the GPU
#
...
LOCAL_PORT="${LOCAL_PORT:-8016}"
VIEWER="${VIEWER:-open -a \"Microsoft Edge\"}"
PROMPT="${PROMPT:-A red apple on a white table, studio lighting}"
SIMULATE_DEPLOY="${SIMULATE_DEPLOY:-true}"

MANIFEST=vllm-omni-image-dgd.yaml
DGD=vllm-omni-image-demo
MODEL=Qwen/Qwen-Image
SELECTOR="nvidia.com/dynamo-graph-deployment-name=${DGD}"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUT_DIR="${REPO_ROOT}/runs/vllm-omni-image-demo"
...
<body>
<h1>Text-to-image on NVIDIA Dynamo with vLLM-Omni</h1>
...
narrate "Text-to-image on NVIDIA Dynamo with vLLM-Omni" \
  "Model: ${MODEL}" \
  "One DynamoGraphDeployment: a CPU frontend plus one H100 worker"

narrate "The worker launches vLLM-Omni with image output:"
pe "sed -n '/command:/,/dynamo_media/p' ${MANIFEST}"
...
pe "jq . request.json"
narrate "20 denoising steps at 512x512 (about 3 s in testing)"
