...
# AI Configurator settings
AIC_SYSTEM="${AIC_SYSTEM:-h100_sxm}"
AIC_HF_ID="Qwen/Qwen3-32B-FP8"
...
generate_dgdr() {
    ...
    case $BACKEND in
        vllm)   runtime_image="nvcr.io/nvidia/ai-dynamo/vllm-runtime:${IMAGE_TAG}" ;;
        sglang) runtime_image="nvcr.io/nvidia/ai-dynamo/sglang-runtime:${IMAGE_TAG}" ;;
        trtllm) runtime_image="nvcr.io/nvidia/ai-dynamo/tensorrtllm-runtime:${IMAGE_TAG}" ;;
...
spec:
  ...
  profilingConfig:
    profilerImage: "${runtime_image}"
    config:
      # SLA targets for airline app
      # - TTFT: Passengers expect quick initial response
      # - ITL: Smooth streaming for rerouting suggestions
      sla:
      ...
      # Use AI Configurator for fast profiling (20-30 seconds)
      sweep:
  ...
  deploymentOverrides:
    workersImage: "${runtime_image}"

  # Auto-deploy after profiling completes
  autoApply: true
EOF
...
deploy_dgdr() {
    ...
    log_phase "🔬" "AI CONFIGURATOR PROFILING" "Finding optimal GPU configuration (typically 20-30 seconds)..."
    ...
    while [[ $elapsed -lt $max_wait ]]; do
        local status
        status=$(kubectl get dgdr "$DEPLOYMENT_NAME" -n "$NAMESPACE" -o jsonpath='{.status.state}' 2>/dev/null || echo "Pending")

        if [[ "$status" != "$last_status" ]]; then
            case $status in
                "Pending")
                    log_info "Status: Pending - Preparing profiling job..."
                    ;;
                "Profiling")
                    log_info "Status: Profiling - AI Configurator analyzing configurations..."
                    ;;
                "Deploying")
                    log_info "Status: Deploying - Creating optimized DynamoGraphDeployment..."
                    ;;
                "Ready")
